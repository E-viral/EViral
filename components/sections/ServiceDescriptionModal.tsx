...
export default function ServiceDescriptionModal({ title, content, onClose }: Props) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4" onClick={onClose} role="dialog" aria-modal="true">
      ...
      <div className="relative bg-white rounded-3xl max-w-2xl w-full max-h-[85vh] overflow-hidden shadow-2xl" onClick={(e) => e.stopPropagation()}
        ...
        <div className="flex items-center justify-between px-8 py-5 border-b border-[#f0efed]">
          ...
          <button onClick={onClose} className="w-9 h-9 rounded-xl bg-[#f4f3f1] flex items-center justify-center hover:bg-[#e2e0dc] transition-colors" aria-label="Close dialog">
            <X size={16} />
          </button>
        </div>
